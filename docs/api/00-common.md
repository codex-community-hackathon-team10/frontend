...
| 인증 | Bearer Access Token + HttpOnly Refresh Token 쿠키 |
| JSON 필드 | `camelCase` |
...
브라우저 클라이언트는 Refresh Token 쿠키 전송을 위해 모든 인증 API 요청에 credentials를 포함한다.
...
- Access Token 만료 시 갱신 요청은 동시에 한 번만 수행한다.
