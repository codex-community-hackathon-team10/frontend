...
원하지 않는 사용자를 차단하고 부적절한 사용자를 신고한다. 차단·신고는 데모 핵심 경로가 완성된 뒤 구현하는 P1 기능이다.
...
```json
{
  "data": {
    ...
    "effects": {
      ...
      "existingAppointmentsCanceled": false
    }
  }
}
```
...
| `context.appointmentId` | X | 신고자가 참여한 약속 ID |
| `context.proposalId` | X | 신고자가 참여한 제안 ID |
...
| `404` | `REPORT_CONTEXT_NOT_FOUND` | 관련 제안·약속 없음 또는 참여자 아님 |
