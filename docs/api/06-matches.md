...
동일 학교·캠퍼스에서 공통 가능 시간이 최소 만남 시간을 충족하는 사용자를 규칙 기반 점수순으로 추천한다. 이 API는 상대의 전체 시간표, 과목명, 강의실, 이메일을 반환하지 않는다.
...
type MatchCandidate = {
  ...
  languageMatch: {
    myLearning: Language[];
    partnerSpeaks: Language[];
    partnerLearning: Language[];
    mySpeaks: Language[];
  } | null;
  score: number;
  reasons: MatchReason[];
